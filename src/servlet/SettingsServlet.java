...
//		softwareName = (String) req.getSession().getAttribute("softwareName");
		...
		String message = "";
		ArrayList<String> importTypesArrayList = new ArrayList<String>(Arrays.asList(importTypes));
		if(oldSettings != null){
			if(importOffice != null && !importTypesArrayList.equals(oldSettings.getImportOffice())){
				message = "Administratie is opgeslagen<br />";
			}if(importTypes != null &&!importTypesArrayList.equals(oldSettings.getImportObjects())){
		...
		if (importTypes != null) {
			...
			if (checkbox != null) {
				...
				if (checkboxes != null) {
					Settings set = new Settings(importOffice, exportOffice, factuurType, checkboxes, user, exportWerkbonType);
					ObjectDAO.saveSettings(set, token);
				}
			}
		}
